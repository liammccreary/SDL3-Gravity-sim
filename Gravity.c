...
// Phys constants
const double GRAVITY = 0.0; // can give it downward gravity if so desired
const double GRAV_CONST = 50.0f; // grav between objs
const double RESTITUTION = 0.8;
const double FRICTION_X = 0.995;
const double DAMPING_COEF = 1.0f;
double hue = 0.0;
...
void trail_render(SDL_Renderer *ren, Trail *t, Uint8 r, Uint8 g, Uint8 b)
{
    ...
    for (int i = 2; i < t->count; i++) {
        SDL_RenderLine(ren,(int)t->x[i - 2], (int)t->y[i - 2], (int)t->x[i], (int)t->y[i]);
    }
}
...
void applyGravity(Ball *a, Ball *b, float deltaTime)
{
    ...
    // Force magnitude
    float force = GRAV_CONST * (a->mass * b->mass) / distSq;
